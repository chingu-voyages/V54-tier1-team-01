...
>>>>>>> Stashed changes:client/src/data/geminiPromptDescriptionInfo.js
      ...
      Do the step one mentioned above. Present the result in markdown format with a clear title, a concise introduction of the pentagram framework (no more than 20 words), an scenario section, and a section shortly describing what the student must do, with some possible things the student should consider for each pentagram component given that scenario - limited to one sentence, and what will happen next. Subtitels of the sections should be always set in (##) markdown.
      `},
      onSubmissionPrompt: (scene)=>{
        /* basic prompt sent to gemini to assess input prompts */
        return `
                You have been previously asked to formulate the following task and scenario for a student: ${scene}. If a prompt is correct, indicate it clearly before commenting it.
