const PROMPTS = {
    test:``,
    onMountScenarioPrompt: (about) => { 
       /* basic prompt sent to gemini to formulate scenario */
       if(about){
        about = "The scenario should be about " + about +'. ';
       }
      return `
      You are a teacher teaching a student how to make prompts using the pentagram framework. Pentagram framework for making a prompt should consists in 5 parts: Persona (the role that the AI has to assume), a Task, the Context of the task, a desired Output format and some Constraints to the output. Your student is a user who is learning prompt engineering but doesn't know how to make prompts based on the pentagram framework. Your task is to provide the student with either a case or an scenario for which he / she should make a composite Pentagram-based prompt. ${about} The resulting prompt based on the scenario provided by you would then be sent to an AI which in turn will provide advice about the quality of that prompt. So it is a training that goes in that order:
      ...
      onSubmissionPrompt: (scene)=>{
        ...
        return `
                ...
                Now it is time to evaluate the prompts made by the student. Provide a concise evaluation in markdown format of no more than 3 sentences per component of the Pentagram prompt framework given by the student. This is the prompt provided by the student separated by Pentagram components:
                    `
      },
